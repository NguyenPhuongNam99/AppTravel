        ...
        return (
          <TouchableOpacity
            ...
            <View style={styles.hotelTop}>
              <Image defaultSource={{uri: 'https://i.pinimg.com/736x/85/03/fa/8503fae3fc4810a28fac2f8913664749.jpg'}} source={{uri: item?.image[0]?.image !== undefined ? item?.image[0]?.image : item?.image[1]?.image}} style={styles.imageSchedule} />
            </View>
            <View style={styles.hotelBottom}>
              <View style={styles.bottomStar}>
                <Text numberOfLines={1} style={styles.colorBottom}>{item.name}</Text>
                {/* <Image source={images.STAR} /> */}
              </View>
              <Text numberOfLines={2} style={styles.fontWeight}>{item.description}</Text>
